...
class CalendarDay extends Component {
  ...
  renderMarking() {
    ...
    if (
      marking.custom &&
      Array.isArray(marking.custom) &&
      marking.custom.length > 0
    ) {
      return this.renderCustom(marking);
    }
  ...
  renderCustom(marking) {
    const baseCustomStyle = [styles.custom, styles.visibleCustom];
    const markedDatesStyle = this.props.markedDatesStyle || {};
    let validCustom = <View style={[styles.custom]} />; // default empty view

    // Filter custom and process only those which have component property
    validCustom = marking.custom
      .filter((d) => d && d.component)
      .map((custom, index) => {
        const Component = custom.component;
        return (
          <View
            key={custom.key ? custom.key : index}
            style={[baseCustomStyle, markedDatesStyle]}
          >
    ...
    return <View style={styles.customContainer}>{validCustom}</View>;
  }
